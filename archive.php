...
<?php if (have_posts()) : ?>
	<?php $post = $posts[0]; // Hack. Set $post so that the_date() works. ?>
		...
		<h2 class="pagetitle">Tag Archive '<?php echo single_cat_title(); ?>'</h2>
		
 		<?php /* If this is a daily archive */ } elseif (is_day()) { ?>
		<h2 class="pagetitle">Daily Archive for <?php the_time('F jS, Y'); ?></h2>
		
		<?php /* If this is a monthly archive */ } elseif (is_month()) { ?>
		<h2 class="pagetitle">Posts no mês de <?php the_time('F, Y'); ?></h2>
		...
		<?php while (have_posts()) : the_post(); ?>
			...
			<div class="post" id="post-<?php the_ID(); ?>">
				<div class="posttitle">
					...
					<p class="post-info">
						Postado em <?php the_category(', ') ?>  on <?php the_time('M jS, Y') ?></p>
				</div>
