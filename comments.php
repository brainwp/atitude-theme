...
<?php if ($comments) : ?>
	<h3 id="comments"><?php comments_number('Sem comentário', 'Um comentário', '% comentários' );?> para &#8220;<?php the_title(); ?>&#8221;</h3> 
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li class="<?php echo $oddcomment; /* Style differently if comment author is blog author */ if ($comment->comment_author_email == get_the_author_email()) { echo ' authorcomment'; } ?>" id="comment-<?php comment_ID() ?>">
			...
			<em>Seu comentário está aguardando aprovação.</em><br />
			<?php endif; ?>			
			<?php comment_text() ?>			
		</li>
 ...
 <?php else : // this is displayed if there are no comments so far ?>
  ...
  <?php if ('open' == $post-> comment_status) : ?> 
	 ...
	 <?php else : // comments are closed ?>
		...
		<p class="nocomments">Comentários são para usuários.</p>
...
<p>Você deve estar <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php the_permalink(); ?>">logado</a> para postar comentários. Você ainda não é usuário? <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=register">Registre-se</a> agora.</p>
<?php else : ?>
...
<label for="email"><small>E-mail (hidden) <?php if ($req) _e('(required)'); ?></small></label></p>
